...
$counter=0;   wp_reset_query() ; the_post(35519); 

// check if the repeater field has rows of data
if( have_rows('jr_pass_type', 35519) ):
// loop through the rows of data
while ( have_rows('jr_pass_type', 35519) ) : the_row();
//if current item == current page - skip

if( have_rows('tour_item', 35519) ):
while ( have_rows('tour_item', 35519) ) : the_row();

if(get_sub_field('id_number', 35519) && get_sub_field('title', 35519)  ){
  $jr_id_list[(string)get_sub_field('title', 35519)]=get_sub_field('id_number', 35519);
  $jr_id_list_reverse[(string)get_sub_field('id_number', 35519)]=get_sub_field('title', 35519);
  $jr_id_text_list .= (string)get_sub_field('id_number', 35519) . "%2C";
}

//$jr .= get_sub_field('id_number', 35519)."\n";
endwhile;
endif;
//array_push($jr_list , "");
endwhile;
...
$jr_name_price=array();
$jr_prices = file_get_contents ( 'https://agent.nx.jtbtravel.com.au/retail-live/rates?ids='.$jr_id_text_list.'&date=2024-01-10' );
...
array_push($jr_list , get_sub_field('category_title', 35519)); 

if( have_rows('tour_item', 35519) ):
while ( have_rows('tour_item', 35519) ) : the_row();
array_push($jr_list ,  get_sub_field('title', 35519));
//$jr .= get_sub_field('id_number', 35519)."\n";
endwhile;
endif;
array_push($jr_list , "");
...
update_option( 'jr_list_popup_2023', $jr_list_popup_text ); 
//add_option('jr_list_3_calc',"@@@");
update_option( 'jr_list_3_calc_2023', $jr_calc_3_passes ); 
