...
//var chkD = chk.getDay();
//if (chkY == this.currYear && chkM == (this.currMonth+1) && i < this.currDay ) {
...
for (var iii = 0;iii< date_array_2.length ; iii++) {
  ...
  if((parseInt(String(chkY)+String(chkM)+String(chkD))<=date_array_2[iii][1])&&(parseInt(String(chkY)+String(chkM)+String(chkD))>=date_array_2[iii][0])){
    ...
    var tempday = new Date( String(chkY)+"-"+String(chkM)+"-"+String(chkD) );
    if((tempday.getDay()==2)&& !((String(date_array_2[iii][2]) == "1")  && (  date_array_2[iii][1] == date_array_2[iii][0]  )    )){ //CLOSE ALL TUESDAYS
      label3=" four";
    }
...
window.onload = function() {
  ...
  c2.showcurr("divCal2");
    ...
    var c3 = new Cal("divCal3");			
  c3.showcurr("divCal3");
  
    var c4 = new Cal("divCal4");     
  c4.showcurr("divCal4");		


    var c5 = new Cal("divCal5");     
  c5.showcurr("divCal5");

   var c6 = new Cal("divCal6");     
  c6.showcurr("divCal6");

var wpfunction = "save_ghib";
var data222 =document.getElementById("ghib-data-2").innerHTML ;
  jQuery.ajax({
  url: 'https://www.nx.jtbtravel.com.au/wp-admin/admin-ajax.php',
  type: "POST",
  cache: false,
  data:{ 
    action: wpfunction, //wp_ajax_save_ghib
    data2: data222 
      },
