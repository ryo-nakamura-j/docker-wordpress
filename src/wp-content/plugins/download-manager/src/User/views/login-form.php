    ...
    <div id="wpdmlogin" <?php if(wpdm_query_var('action') == 'lostpassword') echo 'class="lostpass"'; ?>>
        <?php if(isset($params['logo']) && $params['logo'] != '' && !is_user_logged_in()){ ?>
            <div class="text-center wpdmlogin-logo">
                <a href="<?php echo home_url('/'); ?>"><img alt="Logo" src="<?php echo esc_attr($params['logo']);?>" /></a>
            </div>
        <?php } ?>
        ...
        $('#loginform').submit(function () {
            ...
            $(this).ajaxSubmit({
                error: function(error) {
                    ...
                    $('#loginform').prepend("<div class='alert alert-danger' data-title='<?php _e( "LOGIN FAILED!" , "download-manager" ); ?>'>"+error.responseJSON.message+"</div>");
