    ...
    <div class='w3eden' id='wpdmreg'>
        ...
        if(get_option('users_can_register')){
            ...
            <div class="text-center wpdmlogin-logo">
                <a href="<?php echo esc_url(home_url('/')); ?>"><img src="<?php echo esc_attr($params['logo']);?>" /></a>
            </div>
            <?php } ?>
            ...
            <form method="post" action="" id="registerform" name="registerform" class="login-form">
                ...
                if(!$_social_only){ ?>
                    ...
                    <?php  if(isset($params['note_before']) && trim($params['note_before']) != '') {  ?>
                        <div class="alert alert-info alert-note-before mb-3" >
                            <?php echo esc_html($params['note_before']); ?>
                        </div>
                    <?php } ?>
                ...
                <div class="row">
                    ...
                    if(count($__wpdm_social_login) > 1) { ?>
                        <div class="col-sm-12">
                            <div class="text-center card card-default" style="margin: 20px 0 0 0">
                                <?php if(!$_social_only){ ?>
                                <div class="card-header" data-toggle="collapse" href="#socialllogin" role="button" aria-expanded="false" aria-controls="socialllogin"><?php echo isset($params['social_title'])?esc_html($params['social_title']):__("Or connect using your social account", "download-manager"); ?></div>
                                <?php } else { ?>
                                    <div class="card-header"><?php echo isset($params['social_title'])?esc_html($params['social_title']):__("Connect using your social account", "download-manager"); ?></div>
                                <?php } ?>
                    ...
                    <?php if($loginurl != ''){ ?>
                        <div class="col-sm-12">
                            <br/>
                            <a href="<?php echo esc_attr($loginurl);?>" class="btn btn-link btn-xs btn-block wpdm-login-link color-green" id="registerform-login" name="wp-submit"><?php _e("Already have an account?", "download-manager"); ?> <i class="fa fa-lock"></i> <?php _e( "Login" , "download-manager" ); ?></a>
                        </div>
                    <?php } ?>
                </div>
