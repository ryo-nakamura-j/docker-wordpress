...
class CF7_Material_Design {
	...
	function __construct() {
		...
		if( $this->fs->is__premium_only() ) {
			if( $this->fs->can_use_premium_code() ) {
				add_shortcode( 'md-switch', array( $this, 'md_switch_shortcode__premium_only' ) );
				add_shortcode( 'md-card', array( $this, 'md_card_shortcode__premium_only') );
			}
		}
	...
	public function add_scripts_and_styles() {
		...
		// Scripts
		wp_enqueue_script( 'autosize', plugins_url( '../assets/js/lib/autosize.min.js', __FILE__ ), array(), '1.0', true );
		wp_enqueue_script( 'cf7-material-design', plugins_url( '../assets/js/cf7-material-design-bundle.js', __FILE__ ), array( 'jquery', 'autosize' ), CF7MD_VER, true );

		// Add ajax endpoint for logged-in users to hide the customize link
		if( current_user_can( 'install_plugins' ) ) {
			wp_localize_script( 'cf7-material-design', 'ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
		}
		...
		wp_enqueue_style( 'cf7md_roboto', add_query_arg( $query_args, "//fonts.googleapis.com/css" ), array(), null );
		wp_enqueue_style( 'cf7-material-design', plugins_url( '../assets/css/cf7-material-design.css', __FILE__ ), array(), CF7MD_VER );
		if( $this->fs->can_use_premium_code() ) {
			wp_enqueue_style( 'cf7md-material-icons', '//fonts.googleapis.com/icon?family=Material+Icons', array(), null );
		}
	...
	public function md_form_shortcode( $atts, $content = '' ) {
		...
		?>
		<div id="cf7md-form" class="<?php echo esc_attr( $class ); ?>">
			<div class="mdc-layout-grid">
				<div class="mdc-layout-grid__inner">
					<?php if( is_customize_preview() ) : ?>
						<span class="customize-partial-edit-shortcut"><button aria-label="<?php echo esc_attr( __( "Edit form styles.", 'material-design-for-contact-form-7' ) ); ?>" title="<?php echo esc_attr( __( "Edit form styles.", 'material-design-for-contact-form-7' ) ); ?>" class="customizer-edit customize-partial-edit-shortcut-button" style="cursor: pointer !important;" data-control='{ "name": "cf7md_options[use_custom_styles]" }'><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13.89 3.39l2.71 2.72c.46.46.42 1.24.03 1.64l-8.01 8.02-5.56 1.16 1.16-5.58s7.6-7.63 7.99-8.03c.39-.39 1.22-.39 1.68.07zm-2.73 2.79l-5.59 5.61 1.11 1.11 5.54-5.65zm-2.97 8.23l5.58-5.6-1.07-1.08-5.59 5.6z"></path></svg></button></span>
					<?php endif; ?>
					<?php echo $this->remove_wpautop( $content ); ?>
					<?php if( ! get_transient( 'cf7md_customize_link_closed' ) && current_user_can( 'install_plugins' ) && ! is_customize_preview() ) : ?>
						<div class="cf7md-admin-customize-message mdc-layout-grid__cell mdc-layout-grid__cell--span-12">
							<p>
								<a href="<?php echo $customize_url; ?>" title="<?php echo esc_attr( __( 'Only admins can see this message', 'material-design-for-contact-form-7' ) ); ?>"><?php _e( "Customize your form's colors and fonts", 'material-design-for-contact-form-7' ); ?></a>
								<a href="#" class="cf7md-hide-customize-message" aria-label="<?php echo esc_attr( __( "Don't show this again", 'material-design-for-contact-form-7' ) ); ?>" title="<?php echo esc_attr( __( "Don't show this again", 'material-design-for-contact-form-7' ) ); ?>">&times;</a>
							</p>
						</div>
					<?php endif; ?>
				</div>
	...
	public function md_raw_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item <?php echo esc_attr( $layout_classes ); ?>">
			<?php echo $this->remove_wpautop( $content ); ?>
		</div>
		<?php
	 ...
	 * Card shortcode
	 */
	public function md_card_shortcode__premium_only( $atts, $content = '' ) {
		...
		if( $this->fs->can_use_premium_code() ) {
			...
			?>
			<div class="cf7md-item cf7md-card <?php echo esc_attr( $layout_classes ); ?>">
				<div class="mdc-card">
					<?php if( $title || $subtitle ) : ?>
						<section class="cf7md-card-header">
							<?php if( $title ) : ?>
								<h3 class="cf7md-card-title cf7md-card-title--<?php echo esc_attr( $titlesize ); ?>"><?php echo wp_kses_post( $title ); ?></h3>
							<?php endif; ?>
							<?php if( $subtitle ) : ?>
								<p class="cf7md-card-subtitle"><?php echo wp_kses_post( $subtitle ); ?></p>
							<?php endif; ?>
						</section>
					<?php endif; ?>
	...
	public function md_text_shortcode( $atts, $content = '' ) {
		...
		<div class="cf7md-item cf7md-text <?php echo esc_attr( $layout_classes ); ?>">
			<?php echo $content; ?>
			<?php $this->do_help( $help ); ?>
			<div style="display: none;" class="cf7md-text-html">
				<div class="mdc-text-field <?php echo esc_attr( $modifiers ); ?>">
				<?php if( $this->fs->can_use_premium_code() ) :
						if( $icon ) : ?>
					...
					<?php if( $outlined ) : ?>
						<div class="mdc-notched-outline">
							...
							<div class="mdc-notched-outline__notch">
								<label class="mdc-floating-label"><?php echo wp_kses_post( $label ); ?></label>
							</div>
	...
	public function md_select_shortcode( $atts, $content = '' ) {
		...
		<div class="<?php echo esc_attr( $class ); ?>">
			...
			<div style="display: none;" class="cf7md-select-html">
				<div class="mdc-select <?php echo esc_attr( $modifiers ); ?>">
					<?php if( $this->fs->can_use_premium_code() ) :
						if( $icon ) : ?>
							<i class="material-icons mdc-select__icon"><?php echo wp_kses_post( $icon ); ?></i>
						<?php endif;
					endif; ?>	
	...
	public function md_checkbox_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-checkbox <?php echo esc_attr( $layout_classes ); ?>">
			<?php if( $label !== '' ): ?>
				<label class="cf7md-label cf7md-label--static"><?php echo $is_required ? $label . '*' : $label; ?></label>
			<?php endif; ?>
			<?php echo $content; ?>
	...
	public function md_radio_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-radio <?php echo esc_attr( $layout_classes ); ?>">
			<?php if( $label !== '' ): ?>
				<label class="cf7md-label cf7md-label--static"><?php echo wp_kses_post( $label ); ?></label>
			<?php endif; ?>
			<?php echo $content; ?>
	...
	public function md_accept_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-accept <?php echo $cf7v5_class; echo esc_attr( $layout_classes ); ?>">
			<?php if( $label !== '' ): ?>
				<label class="cf7md-label cf7md-label--static"><?php echo wp_kses_post( $label ); ?></label>
			<?php endif; ?>
			<?php echo $content; ?>
			<label class="cf7md-accept-label"><?php echo $terms; ?></label>
			<div style="display: none;" class="cf7md-checkbox-html">
				<div class="mdc-checkbox__background">
					<svg class="mdc-checkbox__checkmark" viewBox="0 0 24 24">
						<path class="mdc-checkbox__checkmark-path" fill="none" d="M1.73,12.91 8.1,19.28 22.79,4.59"/>
	 ...
	 * Switch shortcode
	 */
	public function md_switch_shortcode__premium_only( $atts, $content = '' ) {
		...
		if( $this->fs->can_use_premium_code() ) {
			...
			<div class="cf7md-item cf7md-switch <?php echo esc_attr( $layout_classes ); ?>">
				<?php if( $label !== '' ): ?>
					<label class="cf7md-label cf7md-label--static"><?php echo $is_required ? $label . '*' : $label; ?></label>
				<?php endif; ?>
				<?php echo $content; ?>
	...
	public function md_file_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-file <?php echo esc_attr( $layout_classes ); ?>">
			<?php if( $label !== '' ): ?>
				<label class="cf7md-label cf7md-label--static"><?php echo $is_required ? $label . '*' : $label; ?></label>
			<?php endif; ?>
			<label class="cf7md-file--label">
				...
				<span class="cf7md-file--value"><?php echo wp_kses_post( $nofile ); ?></span>
				<?php echo $content; ?>
			</label>
			<?php $this->do_help( $help ); ?>
		</div>
	...
	public function md_quiz_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-quiz <?php echo esc_attr( $layout_classes ); ?>">
			<?php if( $label ) : ?>	
				<label class="cf7md-label cf7md-label--static"><?php echo wp_kses_post( $label ); ?></label>
			<?php endif; ?>
			<?php echo $content; ?>
	...
	public function md_captcha_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-captcha <?php echo esc_attr( $layout_classes ); ?>">
			<?php echo $this->remove_wpautop( $content ); ?>
		</div>
		<?php
	...
	public function md_submit_shortcode( $atts, $content = '' ) {
		...
		?>
		<div class="cf7md-item cf7md-submit <?php echo esc_attr( $layout_classes ); ?>" data-button-style="<?php echo esc_attr( $style ); ?>">
			<?php echo $content; ?>
