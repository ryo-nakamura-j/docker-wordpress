...
class CF7_Material_Design_Custom_Style {
	...
	public function add_scripts_and_styles() {
        ...
        if( ! is_admin() &&
            $this->fs->can_use_premium_code() &&
            isset( $this->options['use_custom_styles'] ) &&
            $this->options['use_custom_styles'] &&
            isset( $this->options['font_family'] ) &&
            ! strpos( $this->options['font_family'], 'Roboto' ) ) {
			...
			if( $this->fs->can_use_premium_code() ) {
                ...
                if(
                    ...
                    foreach( $this->options as $key => $option ) {
                        // If it's "use_custom_styles" or "custom_css" we ignore it
                        if( 'use_custom_styles' === $key || 'custom_css' === $key ) continue;

                        // Turn opacity values into rgba
                        if( 'text_hint_on_light' === $key || 'text_on_light' === $key ) {
                            $option = 'rgba(0, 0, 0, ' . $option . ')';
                        } else if( 'text_hint_on_dark' === $key || 'text_on_dark' === $key ) {
                            $option = 'rgba(255, 255, 255, ' . $option . ')';
                        }

                        // Otherwise we print selector { property: value }
                        if( isset( $this->selectors[$key] ) ) {
                            $selectors = $this->selectors[$key];
                            foreach( $selectors as $selector ) {
                                if( 'font-size' === $selector['property'] ) {
                                    $option .= 'px';
                                }
                                $css .= $selector['selector'] . ' { ' . $selector['property'] . ': ' . $option . '; }';
                            }
                        }
                    }
                }
...
#cf7md-form .mdc-radio__native-control:checked + .mdc-radio__background .mdc-radio__outer-circle',
  ...
  #cf7md-form.mdc-theme--dark .mdc-radio__native-control:checked + .mdc-radio__background .mdc-radio__outer-circle',
        ...
        // Button on dark
        $cf7md_selectors['button_on_dark'] = array(
            array(
        ...
        $cf7md_selectors['btn_text_light_theme'] = array(
            array(
        ...
        $cf7md_selectors['btn_text_dark_theme'] = array(
            array(
        ...
        // Text hint on light
        $cf7md_selectors['text_hint_on_light'] = array(
            array( 
                /*'selector' => '#cf7md-form .mdc-textfield__input::-webkit-input-placeholder,
...
#cf7md-form .mdc-textfield__label',
        ...
        // Text hint on dark
        $cf7md_selectors['text_hint_on_dark'] = array(
            array( 
                /*'selector' => '#cf7md-form .mdc-textfield__input--theme-dark::-webkit-input-placeholder,
...
#cf7md-form.mdc-theme--dark .cf7md-help-text',
        ...
        // Text on light
        $cf7md_selectors['text_on_light'] = array(
            array( 
...
#cf7md-form .cf7md-accept-label',
        ...
        // Text on dark
        $cf7md_selectors['text_on_dark'] = array(
            array( 
...
#cf7md-form.mdc-theme--dark .mdc-multi-select:focus .mdc-list-item:checked',
        ...
        // Base font size
        $cf7md_selectors['base_font_size'] = array(
            array(
        ...
        // Font family
        $cf7md_selectors['font_family'] = array(
            array(
