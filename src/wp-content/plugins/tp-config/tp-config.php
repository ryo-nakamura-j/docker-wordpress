...
function tpSetupSession($tp_timeout_idle)
{
  if (isset($tp_timeout_idle) && is_int($tp_timeout_idle) && $tp_timeout_idle > 0)
  {
    ...
    {
      session_start();
	  tp_log('tpSetupSession get last Session [' . session_id() . '] ');
    }
    if (!isset($_SESSION['timeout_idle']))
    {
      session_destroy();
      ...
      $_SESSION = array();
	  tp_log('tpSetupSession generate new Session [' . session_id() . '] ');
    }
...
{
	tp_log('tpSetupSession set tp_timeout_idle Session [' . session_id() . '] ');
...
function tp_is_default_language() {
...
function tp_language_exts() {
	$langs = tp_languages();
	$ext = array();
	array_push( $ext, "" );
	foreach ($langs as $lang) {
		if ( $lang != "" && $lang != tp_system_language() )
			array_push( $ext, $lang );
	}
	return $ext;
}

function tp_cur_language_exts() {
	return tp_current_language() == tp_system_language() ? "" : tp_current_language();
}
...
function tp_extract_pagename( $url ) {
	$toReplace = array('/', '\\', ' ');
	return str_replace( $toReplace, '', $url );
}

function tp_itinerary_url()
{
	return  tp_home_url(false) . '/' .  tp_get_url('tp_itinerary_url', true);
}

function tp_itinerary_pagename() {
	return tp_extract_pagename( tp_get_url('tp_itinerary_url', true) );
}

function tp_checkout_url()
{
	return tp_home_url(!tp_is_default_language()) . '/' .  tp_get_url('tp_checkout_url', true);
}

function tp_checkout_pagename() {
	return tp_extract_pagename( tp_get_url('tp_checkout_url', true) );
}

function tp_payment_url()
{
	return tp_home_url(false) . '/' .  tp_get_url('tp_payment_url', true);
}

function tp_payment_pagename() {
	return tp_extract_pagename( tp_get_url('tp_payment_url', true) );
}

function tp_payment_success_url()
{
	return tp_home_url(false) . '/' .  tp_get_url('tp_payment_success_url', true);
}

function tp_payment_success_pagename() {
	return tp_extract_pagename( tp_get_url('tp_payment_success_url', true) );
}

function tp_payment_failed_url()
{
	return tp_home_url(false) . '/' . tp_get_url('tp_payment_failed_url', true);
}

function tp_payment_failed_pagename() {
	return tp_extract_pagename( tp_get_url('tp_payment_failed_url', true) );
}

function tp_search_url()
{
	return tp_home_url(false) . '/' . tp_get_url('tp_search_url', true);
}

function tp_booking_confirmed_url()
{
	return tp_home_url(!tp_is_default_language()) . '/' . tp_get_url('tp_booking_confirmed_url', true);
	// '/booking-confirmed';
}

function tp_booking_requested_url()
{
	return tp_home_url(!tp_is_default_language()) . '/' . tp_get_url('tp_booking_requested_url', true);
	// '/booking-requested';
}

function tp_booking_failed_url()
{
	return tp_home_url(!tp_is_default_language()) . '/' . tp_get_url('tp_booking_failed_url', true);
	// '/booking-failed';
}
...
function tp_page_logic()
{
	...
	if (is_page()) {
		...
		// Beware that page names are hardcoded here.
		// Only a page contains "checkout" would have the checkout functionality.
		switch($pageObj->post_name) {
			case 'shopping-cart':
			case 'shopping-cart-secure':
				require_once('tp-cart.php');
				tp_shopping_cart_page();
				break;
			case tp_checkout_pagename():
				require_once('tp-checkout.php');
				add_action('wp_head', 'tp_config_head', 5);
				tp_checkout_page();
				break;
			case tp_payment_pagename():
			...
			case 'ipay88-backend':
				...
				break;
			default:
			    tp_log('tp_page_logic add_action tp_config_head ' . $pageObj->post_name);
				add_action('wp_head', 'tp_config_head', 5);
		}
...
function tp_config_inject($cfgparams)
{
	...
	echo 'window.tpSearchParams=' . $searchParams . ';';
	echo "window.tpCurLang='" . tp_cur_language_exts() . "';";
	echo '</script>';
...
function tp_config_head()
{
	...
	$cfgparams = 'serverUrl="' . tp_get_url('tp_app_url') . '"'
	   ...
	   . ' checkoutPage="' . (is_page( tp_itinerary_pagename() ) || tp_page_contains_section("tourplan_itinerary") ? tp_checkout_url()  : '') . '"'
	   . ' currency="' . get_option('tp_currency') . '"'
	   . ' checkoutSynchMs="' . get_option('tp_getCheckoutSynchMs', 5000) . '"'
	   . ' reqTimeout="' . get_option('tp_request_timeout', 20000) . '"';

	if (get_option('tp_payment_alt') === 'true' && get_option('tp_payment_gateway') !== 'none')
	{
	   $cfgparams .= ' checkoutAltPage="' . (is_page( tp_itinerary_pagename() ) && tp_is_payment_alt_cart() ?  tp_checkout_url() . '?tppaymentalt=true' : '') . '"';
	}
    ...
    if (is_page( tp_itinerary_pagename() ) || is_page( tp_checkout_pagename() ))
    {
...
function tp_refresh_servicebuttonconfigs()
{
	...
	{ 
		$json = array();
		if ( array_key_exists($lang, $labelsjson))
			$lbls = $labelsjson[$lang];
		else
			$lbls = $labelsjson[tp_system_language()];
		if ( array_key_exists($lang, $configsjson))
			$cfgs = $configsjson[$lang];
		else
			$cfgs = $configsjson[tp_system_language()];
		foreach ($lbls as $lbl)
		{
			foreach ($cfgs as $cfg)
			{
...
{
    $j = json_decode(tp_read_plugin_file('bookingfees.js'), true);
    checkLastJsonError();
	return $j;
}
...
{
    $j = json_decode(tp_read_plugin_file('deliveryfees.js'), true);
    checkLastJsonError();
	return $j;
}
...
function checkLastJsonError() {
	switch (json_last_error()) {
        ...
        case JSON_ERROR_DEPTH:
            tp_log(' - Maximum stack depth exceeded');
        break;
        case JSON_ERROR_STATE_MISMATCH:
            tp_log(' - Underflow or the modes mismatch');
        break;
        case JSON_ERROR_CTRL_CHAR:
            tp_log(' - Unexpected control character found');
        break;
        case JSON_ERROR_SYNTAX:
            tp_log(' - Syntax error, malformed JSON');
        break;
        case JSON_ERROR_UTF8:
            tp_log(' - Malformed UTF-8 characters, possibly incorrectly encoded');
        break;
        default:
            tp_log(' - Unknown error');
        break;
    }
}
...
add_filter('wp_get_nav_menu_items', 'remove_itinerary_if_empty');

function remove_itinerary_if_empty($items) {
	$newitems = $items;
	if (tpCartIsEmpty()) {
		$itineraryURL = tp_itinerary_url();
		$newitems = array_filter($newitems, function($item) use ($itineraryURL) {
			return trim($item->url, '/') != trim($itineraryURL, '/');
		});
	}
	return $newitems;
}