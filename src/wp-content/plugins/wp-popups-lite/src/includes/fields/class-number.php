 ...
 * @since 1.0.0
 */
class WPPopups_Field_Number extends WPPopups_Field {
	...
	public function init() {
		...
		$this->name  = esc_html__( 'Numbers', 'wp-popups-lite' );
		$this->type  = 'number';
	...
	public function validate( $field_id, $field_submit, $form_data ) {
		...
		if (
			...
			wppopups()->process->errors[ $form_id ][ $field_id ] = wppopups_get_required_label();
		}

		// Check if value is numeric.
		if ( ! empty( $value ) && ! is_numeric( $value ) ) {
			wppopups()->process->errors[ $form_id ][ $field_id ] = apply_filters( 'wppopups_valid_number_label', esc_html__( 'Please enter a valid number.', 'wp-popups-lite' ) );
		}
	}
