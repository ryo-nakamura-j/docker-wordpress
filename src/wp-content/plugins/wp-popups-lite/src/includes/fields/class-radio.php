 ...
 * @since 1.0.0
 */
class WPPopups_Field_Radio extends WPPopups_Field {
	...
	public function init() {
		...
		$this->name     = esc_html__( 'Multiple Choice', 'wp-popups-lite' );
		$this->type     = 'radio';
		...
		add_filter( 'wppopups_field_properties_radio', array( $this, 'field_properties' ), 5, 3 );
	}
	...
	public function field_html_value( $value, $field, $form_data = array(), $context = '' ) {
		...
		if (
			...
			! empty( $field['image'] ) &&
			'entry-table' !== $context &&
			apply_filters( 'wppopups_radio_field_html_value_images', true, $context )
		) {
	...
	public function field_properties( $properties, $field, $form_data ) {
		...
		foreach ( $choices as $key => $choice ) {
			...
			$value = ( '' === $value ) ? sprintf( esc_html__( 'Choice %s', 'wp-popups-lite' ), $key ) : $value;
	...
	public function format( $field_id, $field_submit, $form_data ) {
		...
		if ( 'post_type' === $dynamic && ! empty( $field['dynamic_post_type'] ) ) {
			...
			if ( ! empty( $field['show_values'] ) ) {
				...
				foreach ( $field['choices'] as $key => $choice ) {
					/* translators: %s - choice number. */
					if ( $value_raw === $choice['label'] || $value_raw === sprintf( esc_html__( 'Choice %s', 'wp-popups-lite' ), $key ) ) {
						$choice_key = $key;
						break;
					}
				}
