 ...
 * @since 1.0.0
 */
class WPPopups_Field_Select extends WPPopups_Field {
	...
	public function init() {
		...
		$this->name     = esc_html__( 'Dropdown', 'wp-popups-lite' );
		$this->type     = 'select';
		...
		add_filter( 'wppopups_field_properties_' . $this->type, array( $this, 'field_properties' ), 5, 3 );
	}
	...
	public function field_properties( $properties, $field, $form_data ) {
		...
		if ( ! empty( $field['size'] ) ) {
			$properties['input_container']['class'][] = 'wppopups-field-' . esc_attr( $field['size'] );
		}
