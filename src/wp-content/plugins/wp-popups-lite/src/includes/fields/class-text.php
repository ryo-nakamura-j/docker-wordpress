 ...
 * @since 1.0.0
 */
class WPPopups_Field_Text extends WPPopups_Field {
	...
	public function init() {
		...
		$this->name  = esc_html__( 'Single Line Text', 'wp-popups-lite' );
		$this->type  = 'text';
		$this->icon  = 'fa-text-width';
		$this->order = 30;

		// Define additional field properties.
		add_filter( 'wppopups_field_properties_text', [ $this, 'field_properties' ], 5, 3 );
		add_action( 'wppopups_frontend_js', [ $this, 'frontend_js' ], 10, 1 );
	}
	 ...
	 * @link https://wppopups.com/docs/how-to-use-custom-input-masks/
	 *
	...
	public function field_properties( $properties, $field, $form_data ) {
		...
		if ( ! empty( $field['input_mask'] ) ) {
			...
			$properties['inputs']['primary']['class'][] = 'wppopups-masked-input';
	 ...
	 */
	public function field_display( $field, $deprecated, $popup_data ) {
		...
		if ( isset( $field['limit_enabled'] ) ) {
			...
			$primary['data']['form-id']  = $popup_data['id'];
	...
	public function frontend_js( $popups ) {
		...
		$fields = array_filter(
			$fields,
			function( $field ) {
				return isset( $field['type'] ) && $field['type'] === $this->type && isset( $field['limit_enabled'] ) && ! empty( $field['limit_count'] );
			}
		);
